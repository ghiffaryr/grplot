...
class AnnotateParams:
    def __init__(
        ...
        axes,
        text,
        text_fontsize: Union[float, int] = 0,
    ):
        self.axis = axis
        self.plot = plot
        self.add = add
...
class AnnotateHistBarCountInParetoplot(IAnnotate):
    ...
    def __init__(
        ...
        if (
            (ci is not None)
            or (multiple == "fill")
            or ((hue is not None) and (multiple == "stack"))
        ):
            if "o" in params.text:
        ...
        self.list_height = numpy.array([p.get_height() for p in ax.patches])
        self.list_width = numpy.array([p.get_width() for p in ax.patches])
        ...
        if params.axislabel in params.df:
            if (is_numeric_dtype(params.df[params.axislabel]) == True) or (
                is_numeric_dtype(
                    type(
                        params.df[params.axislabel][
                            first_valid_index(params.df[params.axislabel])
                        ]
                    )
                )
                == True
    ...
    def annotate(self, ax):
        ...
        for p in ax.patches:
            ...
            if (
                (self.list_height.size == 0)
                and (self.list_width.size == 0)
                and (self.numeric != True)
            ):
                is_numeric = False

            if self.axis == "x":
                width_sep = text_sep_type(
                    plot=self.params.plot,
                    df=self.params.df,
                    num=width,
                    sep=self.params.sep,
                    axislabel=self.params.axislabel,
                    axes=self.params.axes,
                )
                width_add = text_add_type(
                    plot=self.params.plot,
                    num=width_sep,
                    add=self.params.add,
                    axislabel=self.params.axislabel,
            ...
            rot, ha, va, xy = self.__build_annotate_param(
                dimension, left, bottom, width, height
            )
            ...
            if (self.params.plot == "paretoplot") and (
                self.text
                in [
                    "h",
                    "h+o",
    ...
    def __build_annotate_param(self, dimension, left, bottom, width, height):
        ...
        if self.axis == "x":
            ...
            delta = (
                max_min_dimension * 0.0175 * self.params.text_fontsize / 10
                if self.params.text_fontsize is None
                else max_min_dimension * 0.0175
            )
        ...
        if self.axis == "y":
            ...
            delta = (
                max_min_dimension * 0.0325 * self.params.text_fontsize / 10
                if self.params.text_fontsize is None
                else max_min_dimension * 0.0325
            )
...
class AnnotateLinePlotInParetoplot(IAnnotate):
    ...
    def __init__(self, params: AnnotateParams):
        self.axis = params.axis
        self.params = params
    ...
    def annotate(self, ax):
        ...
        for data in ax.get_shared_y_axes().get_siblings(ax)[0].get_lines():
            ...
            for x, y in zip(x_arr, y_arr):
                ha, va, add, xy, sibling = self.__build_annotate_params(
                    ax, x, x_arr, y, y_arr
                )

                sibling.annotate(
                    f"{add}", xy=xy, ha=ha, va=va, fontsize=self.params.text_fontsize
                )
    ...
    def __convert_arr_type(self, x_arr, y_arr):
        ...
        if (type(x_arr) == list) or (
            numpy.issubdtype(type(x_arr), numpy.ndarray) == False
        ):
            x_arr = numpy.array(x_arr)

        # check y_arr data type
        if (type(y_arr) == list) or (
            numpy.issubdtype(type(y_arr), numpy.ndarray) == False
        ):
    ...
    def __build_annotate_params(self, ax, x, x_arr, y, y_arr):
        ...
        calc_delta = (
            lambda fontsize, max_val: max_val * 0.0325 * fontsize / 10
        ...
        if self.axis == "x":
            ...
            x_sep = text_sep_type(
                ...
                num=x,
                sep=self.params.sep,
                axislabel="Cumulative Percentage",
                axes=self.params.axes,
            )
            add = text_add_type(
                plot=self.params.plot,
...
class AnnotateScatterResidplot(IAnnotate):
    ...
    def annotate(self, ax):
        ...
        x_arr, y_arr = numpy.array(
            [x for x, _ in ax.collections[0].get_offsets()]
        ...
        # position calibrator
        if self.params.plot == "residplot":
            weight = 5
        else:
            weight = 1

        calc_delta = (
            lambda fontsize, const, max_min: max_min * const * weight * fontsize / 10
            if fontsize is not None
            else max_min * const * weight
        )

        for x, y in ax.collections[0].get_offsets():
            ha, va, add, xy = self.__build_annotate_params(
                x, y, max_y, min_y, calc_delta
            )

            ax.annotate(
                f"{add}", xy=xy, fontsize=self.params.text_fontsize, ha=ha, va=va
            )

        self.ax = ax
        return ax

    def __build_annotate_params(
        self,
        x,
        y,
        max_y,
        min_y,
        calc_delta: Callable[[Union[float, int], float, float], float],
    ):
        ha = va = "center"
        add: Any = None
        delta: float = 0
...
class AnnotateLineEcdfplot(IAnnotate):
    ...
    def annotate(self, ax):
        for data in ax.get_lines():
            ...
            calc_delta = (
                lambda fontsize, max_min, const: max_min * const * fontsize / 10
                if fontsize is not None
                else max_min * const
            )

            for x, y in zip(x_arr, y_arr):
                add, xy, ha, va = self.__build_annotate_params(
                    x, y, max_y, min_y, calc_delta
                )
