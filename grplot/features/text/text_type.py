...
def text_type(plot, df, ax, ci, errorbar, hue, multiple, axis, text, sep, add, text_fontsize, naxislabel, axislabel, axes):
    ...
    if text is None:
        ...
        text_def(plot=plot, df=df, ax=ax, ci=ci, errorbar=errorbar, hue=hue, multiple=multiple, axis=axis, text=text, sep=sep, add=add, text_fontsize=text_fontsize, naxislabel=naxislabel, axislabel=axislabel, axes=axes)
    else:
